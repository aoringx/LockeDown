...
class App:
    ...
    def process_model_output(self, *ofmaps):
        ...
        # Keep only the single highest-score detection (skip NMS)
        if scores.size == 0:
            # No detections: still display the current frame and return state
        ...
        # Compute and display the angle between (nose->left shoulder) and (nose->right shoulder)
        ...
        if ns > 0 and ls > 0 and rs > 0:
            ...
            if n1 > 1e-6 and n2 > 1e-6:
                ...
                cosang = max(-1.0, min(1.0, cosang))  # clamp for numerical safety
        ...
        # Label specific keypoints: nose(0), left shoulder(5), right shoulder(6)
...
def run_mxa(dfp, post_model, app):
    ...
    # Wrap output to capture and print the returned state string
    last_print_time = 0.0
    last_state = None
